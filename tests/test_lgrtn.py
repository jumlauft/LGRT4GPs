...
def test_fit():
    ...
    lgrtn = LGRTN(dx, dy, GP_engine='GPy', lazy_training=True, optimize_hyps = False)
    lgrtn.add_data(xtr,ytr)
    hyps = [lgrtn.kernels[0].parameters[0].values[0],lgrtn.kernels[0].parameters[1].values[0]]
    lgrtn.fit()
    new_hyps =  [lgrtn.kernels[0].parameters[0].values[0],lgrtn.kernels[0].parameters[1].values[0]]
    assert hyps[0] == new_hyps[0]
    assert hyps[1] == new_hyps[1]

    # Verfiy hyps are updated if  wanted
    lgrtn = LGRTN(dx, dy, GP_engine='GPy', lazy_training=True, optimize_hyps = True)
    lgrtn.add_data(xtr,ytr)
    hyps = [lgrtn.kernels[0].parameters[0].values[0],lgrtn.kernels[0].parameters[1].values[0]]
    lgrtn.fit()
    new_hyps =  [lgrtn.kernels[0].parameters[0].values[0],lgrtn.kernels[0].parameters[1].values[0]]
